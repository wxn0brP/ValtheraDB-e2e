...
export const updateTests: TestDefinition[] = [
	{
		...
		fn: async (db: any) => {
			...
			if (updated.length !== 2)
				throw new Error("Expected 2 updated documents, got: " + updated.length);
			if (updated.some((d: any) => d.age !== 100))
				throw new Error("Update did not change all documents");
		},
	},
	{
		domain: "crud-update",
		...
		fn: async (db: any) => {
			...
			if (!Array.isArray(updated))
				throw new Error("update must return an array");
	...
	},
	{
		...
		fn: async (db: any) => {
			...
			if (result !== null)
				throw new Error("updateOne should return null when no match");
		},
	},
];
