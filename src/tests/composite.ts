...
export const compositeTests: TestDefinition[] = [
	{
		...
		fn: async (db: any) => {
			...
			if (result.type !== "updated")
				throw new Error("updateOneOrAdd: expected type=updated");
			if (result.data.val !== 20)
				throw new Error("updateOneOrAdd: document not updated");
		},
	},
	{
		domain: "composite-operations",
		...
		fn: async (db: any) => {
			...
			if (result.type !== "added")
				throw new Error("updateOneOrAdd: expected type=added");
		},
	},
	{
		domain: "composite-operations",
	...
	},
	{
		...
		fn: async (db: any) => {
			...
			if (result.type !== "added")
				throw new Error("toggleOne: expected type=added for new doc");
		},
	},
];
