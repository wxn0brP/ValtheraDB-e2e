...
export const updateOpTests: TestDefinition[] = [
    ...
    },
    {
        ...
        fn: async (db: any) => {
            ...
            if (!result || result.count !== 7) throw new Error("$dec: expected count to be 7, got: " + result?.count);
        },
    },
    {
        domain: "update-operators",
        ...
        fn: async (db: any) => {
            await db.ensureCollection("items");
            await db.add({ collection: "items", data: { _id: "i1", name: "old" } });
            const result = await db.updateOne({
                collection: "items",
                search: { _id: "i1" },
                updater: { $set: { name: "new" } },
            });
            if (!result || result.name !== "new") throw new Error("$set: expected name=new, got: " + result?.name);
        },
    },
    {
        domain: "update-operators",
        ...
        fn: async (db: any) => {
            await db.ensureCollection("items");
            await db.add({ collection: "items", data: { _id: "i1", oldName: "value" } });
            const result = await db.updateOne({
                collection: "items",
                search: { _id: "i1" },
                updater: { $rename: { oldName: "newName" } },
            });
            if (!result || "oldName" in result) throw new Error("$rename: oldName should be removed");
            if (result.newName !== "value") throw new Error("$rename: newName should have the old value");
        },
    },
    {
        domain: "update-operators",
        ...
        fn: async (db: any) => {
            await db.ensureCollection("items");
            await db.add({ collection: "items", data: { _id: "i1", tags: ["a", "b"] } });
            const result = await db.updateOne({
                collection: "items",
                search: { _id: "i1" },
                updater: { $pushSet: { tags: ["b", "c"] } },
            });
            if (!result || !Array.isArray(result.tags) || result.tags.length !== 3) {
                throw new Error("$pushSet: expected 3 tags, got: " + JSON.stringify(result?.tags));
            }
        },
    },
    {
        ...
        fn: async (db: any) => {
            ...
            const result = await db.updateOne({
                collection: "items",
                search: { _id: "i1" },
                updater: { $pushAll: { tags: ["b", "c"] } },
            });
            if (!result || !Array.isArray(result.tags) || result.tags.length !== 3) {
                throw new Error("$pushAll: expected 3 tags, got: " + JSON.stringify(result?.tags));
            }
        },
    },
    {
        ...
        fn: async (db: any) => {
            await db.ensureCollection("items");
            await db.add({ collection: "items", data: { _id: "i1", tags: ["a", "b", "c"] } });
            const result = await db.updateOne({
                collection: "items",
                search: { _id: "i1" },
                updater: { $pull: { tags: "b" } },
            ...
            if (!result || !Array.isArray(result.tags) || result.tags.length !== 2) {
                throw new Error("$pull: expected 2 tags, got: " + JSON.stringify(result?.tags));
            }
            if (result.tags.includes("b")) throw new Error("$pull: b should be removed");
        },
    },
    {
        domain: "update-operators",
        ...
        fn: async (db: any) => {
            await db.ensureCollection("items");
            await db.add({ collection: "items", data: { _id: "i1", tags: ["a", "b", "c", "d"] } });
            const result = await db.updateOne({
                collection: "items",
                search: { _id: "i1" },
                updater: { $pullAll: { tags: ["b", "d"] } },
            });
            if (!result || !Array.isArray(result.tags) || result.tags.length !== 2) {
                throw new Error("$pullAll: expected 2 tags, got: " + JSON.stringify(result?.tags));
            }
    ...
    },
    {
        ...
        fn: async (db: any) => {
            await db.ensureCollection("items");
            await db.add({ collection: "items", data: { _id: "i1", nested: { a: 1, b: { c: 2 } } } });
            const result = await db.updateOne({
                collection: "items",
                search: { _id: "i1" },
                updater: { $deepMerge: { nested: { b: { d: 3 } } } },
            });
            if (!result) throw new Error("$deepMerge: result should exist");
            if (result.nested.a !== 1) throw new Error("$deepMerge: a should be preserved");
            if (result.nested.b.c !== 2) throw new Error("$deepMerge: b.c should be preserved");
            if (result.nested.b.d !== 3) throw new Error("$deepMerge: b.d should be added");
        },
    },
    {
        domain: "update-operators",
    ...
    },
    {
        ...
        fn: async (db: any) => {
            ...
            if (!result || result.count !== 15) throw new Error("$INC: expected count 15, got: " + result?.count);
        },
    },
    {
        domain: "update-operators",
