...
export const removeTests: TestDefinition[] = [
	{
		...
		fn: async (db: ValtheraClass) => {
			...
			if (removed.length !== 2)
				throw new Error("Expected 2 removed documents, got: " + removed.length);
			const remaining = await db.find({
				collection: "users",
			});
			if (remaining.length !== 1)
				throw new Error("Expected 1 remaining document");
		},
	},
	{
		domain: "crud-remove",
		...
		fn: async (db: ValtheraClass) => {
			...
			if (!Array.isArray(removed))
				throw new Error("remove must return an array");
	...
	},
	{
		...
		fn: async (db: ValtheraClass) => {
			...
			if (result !== null)
				throw new Error("removeOne should return null when no match");
		},
	},
];
